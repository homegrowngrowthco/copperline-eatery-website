...
## Live site / current state (2026-08-31)
...
- `scripts/generate-post.mjs` — turns one backlog topic into a post. Two-pass: a draft call grounded in the full menu + `restaurant.ts` facts, then a "humanize" rewrite against `scripts/lib/content-rules.mjs`'s banned-phrase list (modeled on theautomationsguide's real draft-then-humanize pipeline; a single prompt asking an LLM not to sound like an LLM does not hold up). `--force` regenerates a non-"proposed" topic.
- `scripts/blog-gates.mjs` (`npm run qa:blog-gates`) — deterministic post-generation gates. Hard-fails: missing frontmatter, an em/en dash, a missing or denylisted image, a `dishRefs` entry not in `menuData.json`, a wrong phone number. Soft-warns: unmatched dollar amounts, testimonial phrasing, a banned phrase that slipped through both generation passes.
- `scripts/next-backlog-topic.mjs` — prints the next `proposed` topic id for the GHA workflow.
- `.github/workflows/weekly-post.yml` — Monday cron + `workflow_dispatch`; picks a topic, generates, gates, builds, deploys its own Netlify preview (a `GITHUB_TOKEN`-authenticated push does not trigger `deploy.yml`'s `pull_request` preview, so this workflow deploys its own rather than relying on that), opens a **draft PR**. No auto-merge, ever. Needs the `ANTHROPIC_API_KEY` GitHub secret (set) and, as of 2026-08-31, is blocked on a repo setting: "Allow GitHub Actions to create and approve pull requests" is off, so the `gh pr create` step fails until Ian flips it (see `../TODO.md`).
